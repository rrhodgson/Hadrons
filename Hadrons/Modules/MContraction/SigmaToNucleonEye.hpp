...
void TSigmaToNucleonEye<FImpl>::execute(void)
{
    ...
    for (auto &G: Gamma::gall)
    {
      ...
      c=Zero();
      BaryonUtils<FIMPL>::SigmaToNucleonEye(qqLoop,qut,qdTf,qsTi,G,G,GammaB,GammaB,"Q1",c);
      sliceSum(c,buf,Tp);
      r.corr.clear();
      ...
      c=Zero();
      BaryonUtils<FIMPL>::SigmaToNucleonEye(qqLoop,qut,qdTf,qsTi,G,G,GammaB,GammaB,"Q2",c);
      sliceSum(c,buf,Tp);
      r.corr.clear();
      for (unsigned int t = 0; t < buf.size(); ++t)
      {
