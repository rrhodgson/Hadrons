/*
 * BaryonSpinMat.hpp, part of Hadrons (https://github.com/aportelli/Hadrons)
 *
...
#ifndef Hadrons_MContraction_BaryonSpinMat_hpp_
#define Hadrons_MContraction_BaryonSpinMat_hpp_
...
class BaryonSpinMatPar: Serializable
{
public:
    GRID_SERIALIZABLE_CLASS_MEMBERS(BaryonSpinMatPar,
                                    std::string, q1,
                                    std::string, q2,
                                    std::string, q3,
                                    std::string, quarks,
...
class TBaryonSpinMat: public Module<BaryonSpinMatPar>
{
public:
...
public:
    // constructor
    TBaryonSpinMat(const std::string name);
    // destructor
    virtual ~TBaryonSpinMat(void) {};
    // dependency relation
...
MODULE_REGISTER_TMP(BaryonSpinMat, ARG(TBaryonSpinMat<FIMPL>), MContraction);

/******************************************************************************
 *                         TBaryonSpinMat implementation                             *
 ******************************************************************************/
// constructor /////////////////////////////////////////////////////////////////
template <typename FImpl>
TBaryonSpinMat<FImpl>::TBaryonSpinMat(const std::string name)
: Module<BaryonSpinMatPar>(name)
{}

// dependencies/products ///////////////////////////////////////////////////////
template <typename FImpl>
std::vector<std::string> TBaryonSpinMat<FImpl>::getInput(void)
{
...
template <typename FImpl>
std::vector<std::string> TBaryonSpinMat<FImpl>::getOutput(void)
{
...
template <typename FImpl>
std::vector<std::string> TBaryonSpinMat<FImpl>::getOutputFiles(void)
{
...
template <typename FImpl>
void TBaryonSpinMat<FImpl>::parseGammaString(std::vector<GammaABPair> &gammaList)
{
    gammaList.clear();
...
// setup ///////////////////////////////////////////////////////////////////////
template <typename FImpl>
void TBaryonSpinMat<FImpl>::setup(void)
{
    envTmpLat(LatticeComplex, "c");
    envTmpLat(SpinMatrixField, "cMat");
    envTmpLat(SpinMatrixField, "test");
}

// execution ///////////////////////////////////////////////////////////////////
template <typename FImpl>
void TBaryonSpinMat<FImpl>::execute(void)
{
...
*/                          
                ...
                envGetTmp(SpinMatrixField, test);
                test = Zero();

                GridBase *grid = sink.Grid();
  
                  autoView( vtest , test ,CpuWrite);
                  autoView( v1 , sinkTr, CpuRead);
                  autoView( v2 , cMat, CpuRead);

                  accelerator_for(ss, grid->oSites(), grid->Nsimd(), {                    
                    vtest[ss] = v1[ss]*v2[ss]; 
                  }  );//end loop over lattice sites

                sliceSum(test, bufMat, Tp);
...
*/
                ...
                bufMat = sink(cMat);
                // sliceSum(cMat,bufMat,Tp); // temporary
            } 
/*            r.corr.clear();
...
*/
        ...
        std::vector<SpinMatrix> bufMat;
        for (int iG = 0; iG < gammaList.size(); iG++) {
            bufMat = std::vector<SpinMatrix>(nt, Zero());
...
#endif // Hadrons_MContraction_BaryonSpinMat_hpp_
