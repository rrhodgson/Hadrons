...
MODULE_REGISTER_TMP(CoarseDeflationSrcCast   , ARG(TCoarseDeflationSrcCast<FIMPL, CoarseFermionEigenPack<FIMPLF,HADRONS_DEFAULT_LANCZOS_NBASIS>>), MGuesser);
MODULE_REGISTER_TMP(CoarseDeflationSrcCast150, ARG(TCoarseDeflationSrcCast<FIMPL, CoarseFermionEigenPack<FIMPLF,150>>), MGuesser);
MODULE_REGISTER_TMP(CoarseDeflationSrcCast250, ARG(TCoarseDeflationSrcCast<FIMPL, CoarseFermionEigenPack<FIMPLF,250>>), MGuesser);
MODULE_REGISTER_TMP(CoarseDeflationSrcCast400, ARG(TCoarseDeflationSrcCast<FIMPL, CoarseFermionEigenPack<FIMPLF,400>>), MGuesser);
...
public:
  ...
  void operator()(const FineField &src,FineField &guess) { 
    ...
    LOG(Message) << "Total time  : " << time/1e6 << " s" << std::endl;
    LOG(Message) << "blockProject: " << time_project/1e6 << " s" << std::endl;
    LOG(Message) << "blockPromote: " << time_promote/1e6 << " s" << std::endl;
    LOG(Message) << "axpy        : " << time_axpy/1e6 << " s" << std::endl;
  };
};
