...
public:
    ...
    virtual void operator() (const std::vector<FieldOuter> &in, std::vector<FieldOuter> &out)
    {
        ...
        LOG(Message) << "=== Source Cast Guesser START" << std::endl;
        ...
        LOG(Message) << "=== Source Cast Guesser END" << std::endl;
        LOG(Message) << "Total precision change time " << cast_t/1.e6 << " s" << std::endl;
        LOG(Message) << "Total projection time " << guess_t/1.e6 << " s" <<  std::endl;
    }
...
{
    LOG(Message) << "Setting source cast deflation guesser inner guesser '" << par().guesser << "'" << std::endl;
